...
public class FileUploadController {
    ...
    public ResponseEntity<Map<String, String>> uploadFile(@RequestParam("file") MultipartFile file, @RequestParam("recipeName") String recipeName) {
        if (file.isEmpty()) {
            return ResponseEntity.badRequest().body(Map.of("fileName", "", "responseBody", "파일을 선택해 주세요."));
        }
        ...
        try {
            ...
            return ResponseEntity.status(500).body(Map.of("fileName", "", "responseBody", "파일 전송 실패: " + e.getMessage()));
        }
    }
}
